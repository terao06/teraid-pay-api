...
class UserRepository:
    ...
    def get_user_wallet(self, mysql_session: Session, user_id: int) -> Wallet | None:
        """ユーザーに紐づくウォレットを取得します。
        Args:
        ...
        Returns:
            Wallet: ウォレット情報。
        """
        return mysql_session.query(Wallet
        ).join(
    ...
    def get_user_by_id(self, mysql_session: Session, user_id: int) -> User | None:
        """ユーザー ID から店舗情報を取得する。
        ...
        """
        return mysql_session.query(User).where(
            User.user_id == user_id,
            User.deleted_at.is_(None)
        ).one_or_none()

    def create_user_nonce(self, mysql_session: Session, user_nonce: UserNonce) -> None:
    ...
    def create_user_wallet(self, mysql_session: Session, user_wallet: UserWallet) -> None:
        ...
        Returns:
            なし。
        """
        mysql_session.add(user_wallet)
        mysql_session.flush()
        return user_wallet

    def delete_user_nonce_by_nonce_id(self, mysql_session: Session, nonce_id: int) -> None:
        """対象のユーザーnonceを論理削除する。
        ...
        (
            mysql_session.query(UserNonce)
            .where(UserNonce.nonce_id == nonce_id)
            .update({UserNonce.deleted_at: now, UserNonce.updated_at: now})
        )
    
    def delete_user_wallet_by_wallet_id(self, mysql_session: Session, wallet_id: int) -> None:
