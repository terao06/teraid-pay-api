...
VALUES
  ...
  (101, ('[' || array_to_string(ARRAY[0.8] || array_fill(0.0, ARRAY[511]), ',') || ']')::vector, TRUE),
  ...
  (102, ('[' || array_to_string(ARRAY[1.0] || array_fill(0.0, ARRAY[511]), ',') || ']')::vector, TRUE),
  -- nearest face 検索 fixture で返却されない非アクティブ候補として使用
  (103, ('[' || array_to_string(ARRAY[1.0] || array_fill(0.0, ARRAY[511]), ',') || ']')::vector, FALSE),
  -- nearest face 検索 fixture で閾値内検索に一致しない遠い有効候補として使用
  (105, ('[' || array_to_string(ARRAY[0.0, 1.0] || array_fill(0.0, ARRAY[510]), ',') || ']')::vector, TRUE),
  -- test_delete_face_embedding_sets_deleted_at_and_updated_at で削除対象として使用
  (108, ('[' || array_to_string(ARRAY[0.0, 0.5] || array_fill(0.0, ARRAY[510]), ',') || ']')::vector, TRUE),
  -- build_local_db.py で test_face.png から生成した embedding に更新するユーザー
  (107, ('[' || array_to_string(ARRAY[0.0] || array_fill(0.0, ARRAY[511]), ',') || ']')::vector, TRUE);
